...
if __name__ == "__main__":
    ...
    classes = ['smoking']
    label_root = Path(
        "F:\\dc\\yolov5")  # 替换为实际的标注文件夹路径
    image_root = Path(
        "D:\\FilePackage\\datasets\\smoke_5581\\3224_yolo\\images")  # 替换为实际的图像文件夹路径
    output_root = Path(
        "D:\\FilePackage\\datasets\\smoke_5581\\3224_yolo\\yolo2img")  # 替换为实际的输出文件夹路径
    # 创建保存新图像的文件夹
